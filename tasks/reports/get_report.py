    ...
    with self.client.get("/Report",
        ...
        catch_response=True) as response:
            ...
            if response.status_code == 200:
                ...
                print(utility.timestamp(self) + f"Session ID {self.user.session_id}: User '{self.user.username}' Has Retrieved {retrieved_count} Report Type(s)")
    ...
    with self.client.get(f"/Report/{random_report}/Data?filterOperatorList={filters}",
        ...
        catch_response=True) as response:
            ...
            if response.status_code == 200:
                ...
                print(utility.timestamp(self) + f"Session ID {self.user.session_id}: User '{self.user.username}' Has Retrieved {retrieved_count} Document(s) For Report Type ID '{random_report}'")
